...
const SLOTS = [
  ...
  {title:'Palm or Concealed Trick',kind:'trick',intro:'Finish with one effect that interacts with a Catalyst.',points:['Palm and Concealed Tricks have different timing and placement.','Name the Trick for what its effect actually does.','After this step, play a guided battle with registered cards while your drafts remain private.']}
];
...
let tree, cards=SLOTS.map((_,i)=>blank(i)),step=0,complete=false,resetPending=false;

function load() {
  try {
    const stored=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}');
    if(stored.version===1&&Array.isArray(stored.cards)&&stored.cards.length===5){
...
function save() {
  try {localStorage.setItem(STORAGE_KEY,JSON.stringify({version:1,cards,step,complete,savedAt:new Date().toISOString()}));$('save-status').textContent='Draft saved in this browser.';}
...
function renderForm(){
  ...
  $('step-count').textContent=`Step ${step+1} of 5`;
  ...
  $('next-step').textContent=step===4?'Validate all five drafts':'Validate & continue →';
  $('complete-panel').hidden=!complete;
  renderRail();renderPool();
}
function renderRail(){
  $('step-rail').innerHTML=SLOTS.map((slot,i)=>`<li><button class="step-button${i===step?' active':''}${validateCard(i).length===0?' done':''}" type="button" data-step="${i}" aria-current="${i===step?'step':'false'}"><span>${i+1}</span>${esc(slot.title)}</button></li>`).join('');
}
function renderPool(){
...
function onField(event){
  ...
  cards[step][key]=event.target.value;complete=false;
...
function download(){const output={format:'CTF_FIRST_FIVE_FORGE_V1',forgeTreeVersion:'canonical-2026-09-24',exportedAt:new Date().toISOString(),status:'private-draft',cards,validation:validateAll()};const blob=new Blob([JSON.stringify(output,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download='ctf-first-five-forge-draft.json';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);$('save-status').textContent='Draft JSON downloaded.';}
function wire(){
  $('form-fields').addEventListener('input',onField);
  $('form-fields').addEventListener('change',onField);
  $('step-rail').addEventListener('click',event=>{const button=event.target.closest('[data-step]');if(!button)return;step=Number(button.dataset.step);renderForm();save();});
  $('back-step').addEventListener('click',()=>{if(step>0){step--;renderForm();save();}});
  $('next-step').addEventListener('click',()=>{const issues=validateCard(step);if(issues.length){$('form-error').textContent=issues.slice(0,4).join(' · ')+(issues.length>4?` · ${issues.length-4} more issues`:'');return;}if(step<4){step++;renderForm();save();return;}const all=validateAll();if(all.length){$('form-error').textContent=all.slice(0,4).join(' · ')+(all.length>4?` · ${all.length-4} more issues`:'');return;}complete=true;save();renderForm();$('complete-panel').scrollIntoView({behavior:'smooth',block:'nearest'});});
  $('download-draft').addEventListener('click',download);
  $('reset-draft').addEventListener('click',()=>{if(!resetPending){resetPending=true;$('save-status').textContent='Click Start a new draft again to clear the saved five cards.';setTimeout(()=>resetPending=false,8000);return;}resetPending=false;cards=SLOTS.map((_,i)=>blank(i));step=0;complete=false;save();renderForm();});
}
try{
